...
export class GroupSubscriptionsService {
  ...
  async create(data: {chatId : string}, subscriberId: string) {
    ...
    const chat = await checkExistsResurs<GroupChat>(this.prisma, ModelsEnumInPrisma.GROUPT_CHAT, 'id', data.chatId);
  ...
  async findAllSubsriptionsByChatId(id: string) {
    const result = await this.prisma.groupSubscription.findMany({
      where: { chatId: id },
      include : {
        subscriber : {
          include : {
